...
public interface TicketHistoryRepository extends JpaRepository<TicketHistory, Long> {
    ...
    List<TicketHistory> findByIncidentIdOrderByTimestampAscIdAsc(Long incidentId);
    ...
    @Query(value = "SELECT date_trunc('day', \"timestamp\") AS period, COUNT(*) " +
            "FROM ticket_history WHERE ticket_action = :action AND \"timestamp\" >= :from GROUP BY period ORDER BY period",
            nativeQuery = true)
